...
//  baseapp-ios-core-v1
//
//  Created by Pat Sluth on 2026-01-29.
...
extension Alamofire.DataRequest {
    ...
    public func response(_: PMKNamespacer, queue: DispatchQueue = .main)
        -> Promise<(URLRequest, HTTPURLResponse, Data)>
    {
        return Promise { seal in
            response(queue: queue) { rsp in
                if let error = rsp.error {
                    seal.reject(error)
                } else if let a = rsp.request, let b = rsp.response,
                    let c = rsp.data
                {
                    seal.fulfill((a, b, c))
                } else {
...
extension Alamofire.DownloadRequest {
    public func response(
        _: PMKNamespacer,
        queue: DispatchQueue = .main
    ) -> Promise<DownloadResponse<URL?, AFError>> {
        return Promise { seal in
    ...
    public func responseData(
        queue: DispatchQueue = .main
    ) -> Promise<DownloadResponse<Data, AFError>> {
        return Promise { seal in
            responseData(queue: queue) { response in
                switch response.result {
                case .success:
